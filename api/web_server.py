...
class WebServer:
    ...
    def setup_routes(self):
        ...
        def handle_height(val):
            ...
            return jsonify(
                {"status": "error", "message": "Movement engine not ready"}
            ), 503
            ...
            def generate():
                while True:
                    try:
                        if (
                            self.intelligence
                            and not self.intelligence.frame_queue.empty()
                        ):
        ...
        def get_status():
            ...
            if self.intelligence:
                status["mode"] = self.intelligence.context.get(
                ...
                if "battery" in self.intelligence.context["sensors"]:
                    ...
                    if b_data:
                        status["battery"] = {
                            "voltage": b_data.voltage,
                            "percentage": b_data.percentage,
                        }
                ...
                if "last_face" in self.intelligence.context:
                    last_face = self.intelligence.context["last_face"]
                    if last_face:
                        status["detected_face"] = {
                            "name": last_face.get("name", "Unbekannt"),
                            "trust": last_face.get("trust", 0.0),
                        }
        ...
        def set_energy_override(val):
            ...
            if self.intelligence and "mood" in self.intelligence.context:
                try:
                    ...
                    float_val = float(val)
                    self.intelligence.context["mood"].override_energy = (
                        float_val / 100.0
                    )
            ...
            return jsonify(
                {"status": "error", "message": "Intelligence/Mood not ready"}
            ), 503
        ...
        def rename_face():
            ...
            if self.intelligence and hasattr(self.intelligence, "social_memory"):
                success = self.intelligence.social_memory.rename_face(
                    data["id"], data["name"]
                )
                if success:
                    return jsonify({"status": "ok"})
            ...
            return jsonify(
                {"status": "error", "message": "Intelligence not ready"}
            ), 503
        ...
        def update_marker():
            ...
            if not data or "id" not in data or "size" not in data:
                return jsonify(
                    {"status": "error", "message": "Missing id or size"}
                ), 400
