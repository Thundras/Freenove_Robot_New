...
class TestApiModeSwitching(unittest.TestCase):
    def setUp(self):
        ...
        self.intelligence = IntelligenceController(
            config, sensors=self.sensors, gait=self.gait
        )
        self.web = WebServer(
            config, movement_engine=self.gait, intelligence=self.intelligence
        )
        self.app = self.web.app.test_client()

    def find_node(self, name, node=None):
        """Helper to find a node in the BT by name (recursive)"""
        if node is None:
            node = self.intelligence.root

        if node.name == name:
            return node

        # Check children if it's a composite
        ...
        # Check WeightedSelector choices
        if hasattr(node, "choices"):
            for choice in node.choices:
                result = self.find_node(name, choice.node)
                if result:
                    return result

        # Check ConditionDecorator child
        if hasattr(node, "child"):
            result = self.find_node(name, node.child)
            if result:
                return result
    ...
    def test_pose_to_mode_mapping(self):
        ...
        self.assertFalse(
            explore_node.run(), "ExploreRoom should return False when mode is SIT"
        )
        ...
        self.assertFalse(
            explore_node.run(), "ExploreRoom should return False when mode is DOWN"
        )
        ...
        self.assertTrue(
            explore_node.run(), "ExploreRoom should return True when mode is AUTONOMOUS"
        )
    ...
    def test_mode_change_via_api(self):
        ...
        # Verify PlayWithBall (named "BallPlay" in YAML) is suppressed
        ball_node = self.find_node("BallPlay")
        self.assertIsNotNone(ball_node)
        self.assertFalse(ball_node.run(), "BallPlay should return False in MANUAL mode")

        # Verify FollowPerson is suppressed
        follow_node = self.find_node("FollowPerson")
        self.assertIsNotNone(follow_node)
        self.assertFalse(
            follow_node.run(), "FollowPerson should return False in MANUAL mode"
        )
