...
class LegOscillator:
    ...
    def reset(self, phase_offset: float):
        """Reset oscillator to a specific phase offset."""
        self.phase = phase_offset
        self.phase_offset = phase_offset
    ...
    def update(self, dt: float, speed: float):
        ...
        if abs(self.phase_offset - self.target_phase_offset) > 0.001:
            ...
            if diff > 0.5:
                ...
                diff += 1.0
            self.phase_offset = (
                self.phase_offset + diff * self.blending_speed * dt
            ) % 1.0
    ...
    def get_coordinates(
        self, step_length: float, step_height: float, base_y: float
...
class GaitSequencer:
    ...
    def set_gait(self, gait_name: str):
        """Set the gait pattern (trot, walk, idle)."""
        self._apply_gait(gait_name)
    ...
    def set_target_speed(self, speed: float, turn: float = 0.0):
        """Set target speed and turn rate for the gait."""
    ...
    def update(self, dt: float):
        """Update gait state: speed ramping, body pose smoothing, oscillator phases."""
        # 1. Handle Ramping (Speed)
        if self.current_speed < self.target_speed:
            self.current_speed = min(
                self.target_speed, self.current_speed + self.ramp_rate * dt
            )
        ...
        for key in self.current_body_pose:
            ...
            if abs(diff) > 0.01:
                ...
                dist_factor = min(
                    1.0, abs(diff) / 10.0
                )  # Scale down if less than 10 units away
                if key in ["roll", "pitch", "yaw"]:
                    dist_factor = min(1.0, abs(diff) / 5.0)

                # Ease-in/out simulation via distance-based velocity
                actual_step = base_step * (
                    0.2 + 0.8 * math.sin(dist_factor * math.pi / 2)
                )
        ...
        self.auto_com_z = (
            -math.sin(roll_rad) * self.base_height * 0.8
        )  # Counter-shift roll
    ...
    def get_phases(self) -> Dict[str, float]:
        """Return current phase for each leg oscillator."""
    ...
    def calculate_step(self, t: float = 0.0) -> Dict[str, Tuple[float, float, float]]:
        ...
        r = math.radians(
            self.current_body_pose["roll"] + self.stab_roll + self.auto_lean
        )
        p = math.radians(
            self.current_body_pose["pitch"] + self.stab_pitch + self.look_at_pitch
        )
        ...
        # Rz (Pitch) around Z
        Rz = np.array(
            [[math.cos(p), -math.sin(p), 0], [math.sin(p), math.cos(p), 0], [0, 0, 1]]
        )
        ...
        for name, osc in self.oscillators.items():
            ...
            lx, ly, lz = osc.get_coordinates(
                self.step_length, self.step_height, self.base_height
            )

            # B. Compensate for Turning Speed in Gait
            if self.turn_rate != 0:
                leg_speed_mod = 1.0 + (
                    self.turn_rate if name.endswith("l") else -self.turn_rate
                )
                lx, ly, lz = osc.get_coordinates(
                    self.step_length * leg_speed_mod, self.step_height, self.base_height
                )
